...
TEST_CASE("Srp6Math") { // NOLINT
  ...
  Y2KaoZ::Crypto::Srp6::Srp6Math srp6(safePrime, generatorModulo, false);
  ...
  CHECK(privateKey == srp6.calcPrivateKey(salt, username, password));

  const BigInt passwordVerifier("0xe60117ce887dcecedefb4fd6c86cbfdab7bb8b78425b5e6a0bd1562e27ed849");

  CHECK(passwordVerifier == srp6.calcPasswordVerifier(privateKey));
  ...
  CHECK(publicEphemeralValueA == srp6.calcPublicEphemeralValueA(secretEphemeralValueA));
  ...
  CHECK(publicEphemeralValueB == srp6.calcPublicEphemeralValueB(passwordVerifier, secretEphemeralValueB));

  const BigInt randomScramblingParameter("0x81cbdedd03b4196a4452b95c7d01d58548b2acd4");

  CHECK(randomScramblingParameter == srp6.calcRandomScramblingParameter(publicEphemeralValueA, publicEphemeralValueB));

  const BigInt sessionKey("0x6fc506dc9dc2ab2243dea80af42d172d551354ed0015fa3b449a1ed4f122c664");

  const BigInt clientSessionKey =
    srp6.calcClientSessionKey(publicEphemeralValueB, privateKey, secretEphemeralValueA, randomScramblingParameter);

  const BigInt serverSessionKey = srp6.calcServerSessionKey(
  ...
  CHECK(interleavedKey == srp6.calcInterleavedKey(sessionKey));

  const BigInt clientKeyMatchProof("0x7c6ea99648a990565ad1469b00881454b2ae5558");

  CHECK(
    clientKeyMatchProof ==
    srp6.calcClientKeyMatchProof(username, salt, publicEphemeralValueA, publicEphemeralValueB, interleavedKey));

  const BigInt serverKeyMatchProof("0xe06c63642214e078abf045562f44f835ebacbe91");

  CHECK(
    serverKeyMatchProof == srp6.calcServerKeyMatchProof(publicEphemeralValueA, clientKeyMatchProof, interleavedKey));
}
